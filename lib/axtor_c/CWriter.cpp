...
void CWriter::put(const std::string & text)
{
 ...
 */
std::string CWriter::getScalarType(const llvm::Type * type, bool asVectorElementType)
{
#ifdef DEBUG
 ...
 */
std::string CWriter::buildDeclaration(std::string root, const llvm::Type * type)
{
	if (llvm::isa<llvm::ArrayType>(type)) {
		const llvm::ArrayType * arrType = llvm::cast<llvm::ArrayType>(type);
...
std::string CWriter::getFunctionHeader(llvm::Function * func, IdentifierScope * locals)
{
	...
	if (func->hasFnAttribute(llvm::Attribute::InlineHint) || func->hasFnAttribute(llvm::Attribute::AlwaysInline)) {
		builder << "inline ";
	}
 ...
 void CWriter::writeVariableDeclaration(const VariableDesc & desc)
{
 ...
 void CWriter::writeFunctionDeclaration(llvm::Function * func, IdentifierScope * locals)
{
	putLine( getFunctionHeader(func, locals) + ';');
}


std::string CWriter::getInstruction(llvm::Instruction * inst, std::vector<std::string> operands)
{
...
*/
std::string CWriter::dereferenceContainer(std::string root, const llvm::Type * type, AddressIterator *& address, IdentifierScope & locals, const llvm::Type *& oElementType, uint addressSpace)
{
...
std::string CWriter::getVolatilePointerTo(llvm::Value * val, IdentifierScope & locals, const std::string * rootName)
{
...
std::string CWriter::unwindPointer(llvm::Value * val, IdentifierScope & locals, bool & oDereferenced, const std::string * rootName)
{
	uint addressSpace = 0;
...
/*					const llvm::ArrayType * arrType = llvm::cast<llvm::ArrayType>(type);
   ...
   */
   std::string CWriter::getNonInstruction(llvm::Value * value, IdentifierScope & locals)
   {
   ...
   */
std::string CWriter::getShuffleInstruction(llvm::ShuffleVectorInst * shuffle, IdentifierScope & locals)
{
 ...
 */
std::string CWriter::getExtractElementInstruction(llvm::ExtractElementInst * extract, IdentifierScope & locals)
{
...
void CWriter::writeInsertValueInstruction(llvm::InsertValueInst * insert, IdentifierScope & locals)
{
 ...
 */
void CWriter::writeInsertElementInstruction(llvm::InsertElementInst * insert, IdentifierScope & locals)
{
...
*/
std::string CWriter::getInstructionAsExpression(llvm::Instruction * inst, IdentifierScope & locals)
{
...
*/
 void CWriter::writeFunctionHeader(llvm::Function * func, IdentifierScope * locals)
{
  putLine( getFunctionHeader(func, locals));
}

 void CWriter::writeInstruction(const VariableDesc * desc, llvm::Instruction * inst, IdentifierScope & locals)
{
#ifdef DEBUG
...
void CWriter::writeIf(const llvm::Value * condition, bool negate, IdentifierScope & locals)
{
 ...
 */
void CWriter::writePostcheckedWhile(llvm::BranchInst * branchInst, IdentifierScope & locals, bool negate)
{
 ...
 */
 void CWriter::writePrecheckedWhile(llvm::BranchInst * branchInst, IdentifierScope & locals, bool negate, InstructionSet * oExpressionInsts)
{
  llvm::Value * loopCond = branchInst->getCondition();
 ...
 void CWriter::writeInfiniteLoopBegin()
{
   putLine ( "while(true)" );
}

 void CWriter::writeInfiniteLoopEnd()
{
   putLine( "" );
}

 void CWriter::writeReturnInst(llvm::ReturnInst * retInst, IdentifierScope & locals)
	{
...
void CPassThroughWriter::put(const std::string &  msg)
{
	parent.put(msg);
}
